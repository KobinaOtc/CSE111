...
from water_flow import water_column_height, pressure_gain_from_water_height,\
      ...
      pressure_loss_from_pipe_reduction, convert_to_psi
import pytest

def test_water_column_height():
    '''
    This test function must call water_column_height 
    at least four times to verify that it is working 
    correctly.
    '''
...
def test_pressure_gain_from_water_height():
    '''
    This test function must call pressure_gain_from_water_height 
...
def test_convert_to_psi():
    assert convert_to_psi(158.7) == approx(23.017489, abs=0.001)
    assert convert_to_psi(-158.7) == approx(-23.0174889, abs=0.001)
    assert convert_to_psi(352) == approx(51.0533, abs=0.001)
